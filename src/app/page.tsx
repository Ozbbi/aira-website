...
// CLERK_READY constant from next.config.mjs) — this component only renders
// Clerk hooks when layout actually mounted <ClerkProvider>.
...
/* Real Google login via Clerk — only ever rendered when CLERK_ON, so useUser/
   useClerk always run inside a mounted <ClerkProvider>. Clicking opens Clerk's
   hosted sign-in (Google works out of the box on a Clerk dev instance — no
   separate Google Cloud Console setup needed). */
